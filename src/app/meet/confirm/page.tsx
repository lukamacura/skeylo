...
export default function MeetConfirmPage() {
  ...
  const slotLocalQP = searchParams.get("slotLocal") ?? ""; // opciono ako šalješ već formatiran string

  // Lepi label – računamo klijentski da izbegnemo SSR mismatch
  ...
  function downloadICS() {
    ...
    const start = new Date(slotISO); // UTC
    const end = new Date(start.getTime() + 60 * 60 * 1000); // podrazumevano 60 min

    // dt stamp helper (UTC, basic format)
    const fmt = (d: Date) =>
      d
        .toISOString()
        .replace(/[-:]/g, "")
        ...
        <Card className="mx-auto mt-8 max-w-2xl border border-primary/40 bg-transparent text-foreground">
          <CardContent className="space-y-6 p-4 md:p-6">
            ...
            <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button
                onClick={downloadICS}
                className="w-full sm:w-auto text-background cursor-pointer font-semibold"
                size="lg"
              >
