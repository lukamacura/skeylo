    ...
    <section className="py-20">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="relative mt-12 grid gap-10 md:grid-cols-3">
          ...
          <div className="pointer-events-none absolute left-0 right-0 top-24 hidden h-px md:block bg-border" />
          ...
          {steps.map(({ title, copy, img, FallbackIcon }, i) => (
            <motion.article
              ...
              className="relative z-10 rounded-2xl border bg-background/70 p-6 shadow-sm"
            >
              {/* brojač + “balon” */}
              <div className="mb-4 flex items-center gap-3">
                <div className="grid size-9 place-items-center rounded-full border bg-background font-medium">
                  {i + 1}
                </div>
              ...
              <div className="relative mb-4 aspect-[4/4] w-full overflow-hidden rounded-xl border bg-muted/30">
                {img ? (
                  <Image
                    src={img}
                    alt={title}
              ...
              {/* strelica ka sledećem (md+) */}
              {i < steps.length - 1 && (
                <div className="hidden md:block">
                  <svg
                    className="absolute right-[-28px] top-24 h-6 w-6 text-muted-foreground/50"
                    viewBox="0 0 24 24"
                    fill="none"
                    aria-hidden
                  >
                    <path
                      d="M3 12h16m0 0l-4-4m4 4l-4 4"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      strokeLinecap="round"
